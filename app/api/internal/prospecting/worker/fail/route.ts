...
import { WorkerFailRequestSchema } from "@/lib/prospecting/types";
...
export async function POST(request: Request) {
  ...
  const parsed = WorkerFailRequestSchema.safeParse(rawJson);
  if (!parsed.success) {
    return NextResponse.json(
      { error: "Invalid failure payload", details: parsed.error.flatten().fieldErrors },
      { status: 400 }
    );
  }

  const result = globalProspectingLeaseManager.failJob(parsed.data);
