...
export async function POST(request: Request) {
  ...
  const response = await fetch(functionUrl, {
    method: "POST",
    headers: { "Content-Type": "application/json", "x-alastre-user-email": email, "x-alastre-bridge-secret": bridgeSecret, "x-alastre-write-mode": process.env.ALASTRE_WRITE_MODE ?? "disabled" },
