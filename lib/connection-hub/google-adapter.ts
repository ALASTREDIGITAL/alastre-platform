import "server-only";
import type {GoogleOAuthConfig} from "./server-config";
export const GOOGLE_BUSINESS_SCOPE="https://www.googleapis.com/auth/business.manage";
const AUTH_URL="https://accounts.google.com/o/oauth2/v2/auth",TOKEN_URL="https://oauth2.googleapis.com/token",ACCOUNTS_URL="https://mybusinessaccountmanagement.googleapis.com/v1/accounts",BUSINESS_INFO_URL="https://mybusinessbusinessinformation.googleapis.com/v1";
...
async function json(response:Response){const body=await response.json().catch(()=>({}));return record(body)?body:{}}
function translate(status:number,body:Record<string,unknown>){const code=typeof body.error==="string"?body.error:status===401?"authorization_revoked":status===403?"google_access_denied":status>=500?"google_unavailable":"google_request_failed";return new GoogleProviderError(code,status,status===401||code==="invalid_grant");}
function sanitizeMetadata(value:Record<string,unknown>){return Object.fromEntries(Object.entries(value).filter(([key,item])=>["canHaveFoodMenus","canOperateHealthData","hasGoogleUpdated","hasPendingEdits","hasVoiceOfMerchant"].includes(key)&&typeof item==="boolean"));}
function sanitizeHours(value:Record<string,unknown>){const periods=Array.isArray(value.periods)?value.periods.filter(record).map(period=>({openDay:typeof period.openDay==="string"?period.openDay:undefined,openTime:record(period.openTime)?period.openTime:undefined,closeDay:typeof period.closeDay==="string"?period.closeDay:undefined,closeTime:record(period.closeTime)?period.closeTime:undefined})).slice(0,21):[];return {periods};}
