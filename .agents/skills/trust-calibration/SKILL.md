...
For Alastre, classify material claims as verified fact, sourced data, inference, partial data, suggestion, unknown, or unavailable. Confidence follows evidence coverage and must never be fabricated as a cosmetic percentage. `N/D` is not zero, and a polished presentation must not imply that an unverified result is real.
...
- **Source visibility** — whether the AI shows evidence, concise rationale, sources, or alternatives
...
- **Alternative surfacing**: "Two interpretations: A and B. I went with A because…" — shows the decision basis without exposing private reasoning
