...
For Alastre, ambient signals may inform the current interaction, but they must not silently change critical rules, permissions, tenant boundaries, approval gates, campaign behavior, publishing policy, or security controls. Persistent learning requires a declared purpose, traceable source, user-visible effect, and human review when it can affect external actions.
## Closing the Loop
...
- **Governed persistence**: Separate a preference from an operational rule; record who changed it and allow review or reversal
