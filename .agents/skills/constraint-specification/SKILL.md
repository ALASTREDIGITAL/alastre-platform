...
For Alastre, translate critical constraints into observable acceptance criteria and server-side enforcement where applicable. The priority is: user request within current authority → project instructions and architecture decisions → `alastre-security-review` → domain rules → stylistic preferences. Prompt constraints do not replace authorization, RLS, backend validation, idempotency, or approval gates.
## Design Artefacts
