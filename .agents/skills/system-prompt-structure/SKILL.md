...
- Interaction style (ask clarifying questions only when needed, be concise, and provide a verifiable summary of relevant checks)
...
For Alastre agents, include explicit tenant scope, allowed data, prohibited effects, approval boundary, uncertainty behavior, audit requirements, prompt version, and safe failure state. Do not request, store, or expose private chain-of-thought; expose evidence, assumptions, validations, decisions, and concise rationale instead.
