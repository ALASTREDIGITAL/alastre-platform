...
This is an automatically discoverable, read-only audit when the user says a screen feels static or asks where motion would improve continuity. Apply the `alastre-motion-system` purpose gate and return no more than five high-confidence opportunities plus deliberate rejections. Never change source, create files, install packages, or suggest motion merely to decorate the dashboard.
