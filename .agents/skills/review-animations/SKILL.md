...
Allow automatic routing only for explicit motion-review requests. Review purpose, frequency, motion tokens, reduced motion, readability, interruptibility, layout shift and GPU cost. `alastre-motion-system` and `alastre-product-ux` override rigid external values. Report evidence in the conversation; do not mutate code unless the user also requests fixes.
