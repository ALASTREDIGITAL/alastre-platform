...
Apply motion only after hierarchy and UX are resolved. Every animation must orient attention, explain change, connect origin and destination, confirm interaction, or improve perceived quality; otherwise omit it. Reuse `alastre-motion-system` tokens, prefer CSS with `transform` and `opacity`, avoid new libraries, and ship a meaningful `prefers-reduced-motion` alternative. No continuous pulse/glow, gratuitous bounce, normal-operation confetti, decorative data motion, or animation that delays reading.
