...
For Alastre templates, type and bound tenant identifiers, data sources, confidence, prompt version, permitted operation, approval state, and output schema. Treat missing critical variables as a safe error rather than filling them with invented values. Template rendering never grants permission for an external action.
## Design Artefacts
