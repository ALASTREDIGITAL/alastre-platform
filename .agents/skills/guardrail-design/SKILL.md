...
In the Alastre Platform, apply `alastre-security-review` as the higher-priority enforcement profile. A prompt-level guardrail is defense in depth, not a substitute for server-side authorization, RLS, approval gates, idempotency, audit, or `ALASTRE_WRITE_MODE`. External writes and destructive operations remain hard-blocked until the specific action is explicitly authorized.
