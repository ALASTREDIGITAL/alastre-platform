...
## Alastre profile — higher priority

Treat Alastre as a high-frequency SaaS productivity dashboard: restraint and production polish are primary; playful experimentation is excluded except for rare, low-stakes empty/onboarding states. Always report inline in Markdown. Do not create or open HTML reports, auto-loop demos, files, browser windows, or dependencies. Do not stop for designer-weight confirmation when product context is already documented. Use `alastre-motion-system` as the source of semantic tokens and `alastre-product-ux` as the authority for readability and navigation.
