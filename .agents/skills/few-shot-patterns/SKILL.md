...
For Alastre, examples must use fictional or sanitized values, label missing evidence honestly, and cover safe refusal or handoff for external writes. Examples demonstrate behavior but never override current tenant scope, permissions, or approval state. Include a bad example only when it is clearly labeled and cannot be mistaken for a valid instruction.
