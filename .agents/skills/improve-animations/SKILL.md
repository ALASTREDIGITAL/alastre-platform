...
For the Alastre Platform, this skill audits and recommends in the conversation. Do not create `plans/` files, dispatch subagents, install dependencies, commit, or mutate source unless the user explicitly asks for that separate action. Do not pause for selection during an audit: return at most five vetted, prioritized findings with `file:line`, purpose, frequency, exact token role, reduced-motion behavior, and a feel-check. `alastre-motion-system` and `alastre-product-ux` override conflicting values or stylistic advice.
