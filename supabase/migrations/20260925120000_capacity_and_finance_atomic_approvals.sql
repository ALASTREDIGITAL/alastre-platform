-- ==============================================================================
-- Migration: 20260925120000_capacity_and_finance_atomic_approvals.sql
-- Módulo 08: Submissão e Aprovação Atômica com Consistência Estrita de Proposta e Cliente
--
-- Objetivos:
-- 1. RPC submit_capacity_pricing_proposal_for_approval para criação atômica no banco
--    (approval_items + financial_pricing_decisions + audit_events em transação única)
-- 2. RPC process_capacity_pricing_approval atualizada com verificação de consistência:
--    - approval_items.source_type = 'capacity_financial_pricing'
--    - approval_items.source_id::text = financial_pricing_decisions.proposal_id
--    - pertencimento à mesma agência autenticada
--    - verificação de status pendente em ambas as tabelas
-- 3. RLS e privilégios restritos exclusivamente ao service_role
-- ==============================================================================

-- 1. RPC Transacional de Submissão Atômica de Precificação
...
begin
  ...
  -- Tenta converter proposal_id para UUID se for formato UUID, ou gera UUID determinístico para a tabela approval_items
  begin
    v_source_uuid := p_proposal_id::uuid;
  exception when invalid_text_representation then
    v_source_uuid := gen_random_uuid();
  end;

  -- 3. Inserção atômica em approval_items
...
-- 2. Atualização da RPC Transacional process_capacity_pricing_approval com Consistência de Proposta
...
begin
  ...
  -- 5. VALIDAÇÃO DE CONSISTÊNCIA E DIVERGÊNCIA DE PROPOSTA
  -- Confirma que o proposal_id da decisão bate com a proposta no snapshot/source do item
  if (v_approval.snapshot->>'proposal_id') is not null and (v_approval.snapshot->>'proposal_id') <> v_pricing.proposal_id then
